...
namespace inari {
    ...
    {
        const Color::Clamped glColor = color.getClampedColor();
        glClearColor(static_cast<float>(glColor.r) / 255.0f, static_cast<float>(glColor.g) / 255.0f,
                     static_cast<float>(glColor.b) / 255.0f, static_cast<float>(glColor.a) / 255.0f);
